<template>
  <div class="w-full h-96 bg-white rounded-xl shadow-xl p-8 border border-blue-100">
    <h3 class="text-2xl font-bold text-blue-600 mb-6">{{ title }}</h3>
    <div ref="chartContainer" class="w-full h-[380px] bg-gradient-to-b from-blue-50 to-white rounded-lg"></div>
  </div>
</template>
...
const props = defineProps({
  ...
  title: { type: String, default: 'YTD Analysis (14 produits)' }
})
...
const renderChart = () => {
  ...
  props.data.forEach(row => {
    ...
    if (monthIdx >= 0 && productId) {
      ...
      dataByProduct[productId][monthIdx] = Number(row.anaytd || 0)
    }
  })
  
  // 🔥 TOUS les 14 produits
  ...
  console.log('🔵 ANA - Produits:', allProducts)
