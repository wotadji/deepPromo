    ...
    config(
        ...
        pre_hook="
            {%- if adapter.get_relation(this.database, this.schema, this.identifier) -%}
                drop index if exists {{ this.schema }}.{{ this.name }}_idx;
            {%- endif -%}
        ",
        post_hook=[
            "create index if not exists {{ this.name }}_idx on {{ this }} (id_items, last_updated_date)",
            "analyze {{ this }}"
        ]
    ) 
}}
WITH stg_sales_data AS 
    (SELECT
        CAST(id_items AS INT) AS id_items,
    ...
    FROM
        {{ source("raw_deepPromo", "dp_lake_parquet") }}
)
